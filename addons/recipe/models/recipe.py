...
class Recipe(models.Model):
    ...
    def action_consolidate_recipe(self):
        """Merge duplicate ingredients with the same name and compatible units,
        normalize units, and resequence lines."""
        self.ensure_one()
        ...
        # Apply consolidation: keep first occurrence per group, update it; unlink the rest
        to_unlink = self.env['recipe.ingredient']
        seen = set()
        for line in self.ingredient_ids:
            name_key = (line.name or '').strip().lower()
            unit_key = self._normalize_unit(line.unit)
            ...
            line.write({
                ...
                'unit': unit_key,
                'notes': ', '.join(data['notes']) if data['notes'] else line.notes,
            })

        if to_unlink:
            to_unlink.unlink()

        # Resequence by original first appearance preserving current sorted order
        sequence = 10
        for key in order:
            name_key, unit_key = key
            record = self.ingredient_ids.filtered(lambda r: (r.name or '').strip().lower() == name_key and self._normalize_unit(r.unit) == unit_key)[:1]
            if record:
...
class RecipeConsolidateWizard(models.TransientModel):
	_name = 'recipe.consolidate.wizard'
	_description = 'Consolidate Recipe Wizard'

	recipe_id = fields.Many2one('recipe.recipe', string='Recipe', required=True)

	def action_consolidate(self):
		self.ensure_one()
		self.recipe_id.action_consolidate_recipe()
		return {
			'type': 'ir.actions.act_window',
			'name': 'Recipe',
			'res_model': 'recipe.recipe',
